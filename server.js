...
app.get("/users", auth, async (req,res)=>{
  try{
    ...
    if(req.user.role==="superadmin"){
      ...
      users = await User.find({ agencyId: req.user.agencyId }).populate("agencyId").populate("stores");
    }
    res.json(users);
  }catch(err){
...
app.put("/users/:id/agency", auth, onlySuper, async (req,res)=>{
  try{
    ...
    user.agencyId = req.body.agencyId || null;
    await user.save();
...
app.put("/users/:id/stores", auth, onlyAdmin, async (req, res) => {
  try {
    ...
    // Validar que los stores existan
    const validStores = await Store.find({ _id: { $in: storeIds } });
    userToUpdate.stores = validStores.map(s => s._id);
